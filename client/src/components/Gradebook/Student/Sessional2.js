...
export default function Sessional2({ currentStudent }) {
  ...
  const totalMarks = subjects.reduce((total, subject) => {
    ...
    return (
      total +
      (parseInt(
        sessional2Data.marksLecture[subject.name]?.reduce(
          (acc, mark) => acc + mark,
          0
        )
      ) || 0)
  ...
  const totalPossibleMarks = subjects.reduce((total, subject) => {
    return total + (parseInt(sessional2Data.totalExamMarks[subject.name]) || 0);
  }, 0);
  ...
  const totalAttendance = subjects.reduce((total, subject) => {
    return (
      total +
      (parseInt(sessional2Data.attendanceLecture[subject.name]) || 0) +
      (parseInt(sessional2Data.attendanceLab[subject.name]) || 0)
    );
  }, 0);
  ...
  const totalPossibleAttendance = subjects.reduce((total, subject) => {
    const lectures =
      parseInt(sessional2Data.totalAttendanceLecture[subject.name]) || 0;
    const labs = parseInt(sessional2Data.totalAttendanceLab[subject.name]) || 0;
  ...
  return (
    <>
      <table>
        <thead>
          <tr>
            <th colSpan="8">Sessional 2</th>
          </tr>
          <tr>
            <th style={{ width: "5%" }} colSpan="1">
              Subject Code
            </th>
            <th style={{ width: "20%" }} className="wid" colSpan="1">
              Subject Name
            </th>
            ...
            <th className="wid" colSpan="1">
              Lecture Attendance Out Of
            </th>
            <th className="wid" colSpan="1">
              Lab Attendance
            </th>
            <th className="wid" colSpan="1">
              Lab Attendance Out Of
            </th>
          </tr>
        </thead>
        <tbody>
          {subjects.map((subject, index) => (
            <tr key={index}>
              ...
              <td>
                {sessional2Data.marksLecture[subject.name]?.join(", ") || ""}
              </td>
              <td>{sessional2Data.totalExamMarks[subject.name]}</td>
              <td>{sessional2Data.attendanceLecture[subject.name]}</td>
              <td>{sessional2Data.totalAttendanceLecture[subject.name]}</td>
              <td>{sessional2Data.attendanceLab[subject.name]}</td>
              <td>{sessional2Data.totalAttendanceLab[subject.name]}</td>
            </tr>
          ))}
        </tbody>
      </table>
